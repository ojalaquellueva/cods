...
# Probably remove this entire block
# Users and permissions can be executed separately after 
# build db using script setp.sh	
: <<'COMMENT_BLOCK_1'

echoi $e -n "Changing owner to 'bien'..."
sudo -Hiu postgres PGOPTIONS='--client-min-messages=warning' psql --set ON_ERROR_STOP=1 -q -c "ALTER DATABASE $DB OWNER TO bien" 
source "$includes_dir/check_status.sh"  
...
GRANT CONNECT ON DATABASE $DB TO bien;
GRANT CONNECT ON DATABASE $DB TO public_bien;
GRANT ALL PRIVILEGES ON DATABASE $DB TO bien;
\c $DB
...
echoi $e "Importing political division tables from DB \"$DB_GEOM\":"
...
PGOPTIONS='--client-min-messages=warning' psql -d $DB --set ON_ERROR_STOP=1 -q -f $DIR/sql/create_core_tables.sql
source "$includes_dir/check_status.sh"  
...
PGOPTIONS='--client-min-messages=warning' psql -d $DB --set ON_ERROR_STOP=1 -q -f $DIR/sql/create_centroid_country.sql
source "$includes_dir/check_status.sh"  

echoi $e -n "- centroid_country_multi..."
PGOPTIONS='--client-min-messages=warning' psql -d $DB --set ON_ERROR_STOP=1 -q -f $DIR/sql/create_centroid_country_multi.sql
source "$includes_dir/check_status.sh"  

echoi $e -n "- centroid_state_province..."
PGOPTIONS='--client-min-messages=warning' psql -d $DB --set ON_ERROR_STOP=1 -q -f $DIR/sql/create_centroid_state_province.sql
source "$includes_dir/check_status.sh"  

echoi $e -n "- centroid_county_parish..."
PGOPTIONS='--client-min-messages=warning' psql -d $DB --set ON_ERROR_STOP=1 -q -f $DIR/sql/create_centroid_county_parish.sql
source "$includes_dir/check_status.sh"  
