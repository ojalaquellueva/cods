...
if [ -z ${master+x} ]; then
	...
	pname="Set permissions for database $DB"
...
if [[ "$USER_ADMIN" == "" ]]; then
	...
	sudo -Hiu postgres PGOPTIONS='--client-min-messages=warning' psql --set ON_ERROR_STOP=1 -q -c "ALTER DATABASE $DB OWNER TO $USER_ADMIN" 
	...
	GRANT ALL PRIVILEGES ON DATABASE $DB TO $USER_ADMIN;
	\c $DB
...
EOF
	...
	echoi $e -n "--- Tables..."
#	for tbl in `psql -qAt -c "select tablename from pg_tables where schemaname='public' and tableowner<>'postgres';" $DB` ; do  sudo -Hiu postgres PGOPTIONS='--client-min-messages=warning' psql -q -c "alter table \"$tbl\" owner to $USER_ADMIN" $DB ; done
	...
	for tbl in `psql -qAt -c "select tablename from pg_tables where schemaname='public' and tablename not in ('spatial_ref_sys');" $DB` ; do  sudo -Hiu postgres PGOPTIONS='--client-min-messages=warning' psql -q -c "alter table \"$tbl\" owner to $USER_ADMIN" $DB ; done
	source "$includes_dir/check_status.sh"  
...
if [[ ! "$USER_READ" == "" ]]; then
	...
	\set ON_ERROR_STOP on
	REVOKE CONNECT ON DATABASE $DB FROM PUBLIC;
	GRANT CONNECT ON DATABASE $DB TO $USER_READ;
	\c $DB
