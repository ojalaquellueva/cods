...
# keyword algorithm
#  
# Purpose: Flags observations as potentially cultivated if the specimen or
# 	locality descriptions contain keywords indicating farm, plantation, etc.
#
# Usage: ./cods_key.sh
#
...
if [ "$i" == "true" ]; then
	...
	Run process '$pname_key' using the following parameters: 
...
tbl_user_data="user_data_key_${uniqid}"
...
cmd="$opt_pgpassword PGOPTIONS='--client-min-messages=warning' psql $opt_user -d $DB --set ON_ERROR_STOP=1 -q -v tbl_user_data_raw="${tbl_user_data_raw}" -f $DIR_LOCAL/sql/create_raw_data_temp_key.sql"
eval $cmd
...
echoi $e -n "Searching for keywords..."
cmd="$opt_pgpassword PGOPTIONS='--client-min-messages=warning' psql $opt_user  -d $DB --set ON_ERROR_STOP=1 -q -v tbl_user_data_raw=${tbl_user_data_raw} -v dist_threshold=${dist_threshold} -f $DIR_LOCAL/sql/keyword.sql"
eval $cmd
source "$DIR/includes/check_status.sh"
